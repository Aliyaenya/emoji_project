...
from .states import UserMessageState  #.этот файл из этой папки
...
async def welcome_message(message:types.Message):
    text = """
        ...
        Чтобы начать игру, отправь мне соообщение /start_game
    """
...
async def start_with_category(call: types.CallbackQuery, state: FSMContext):
    ...
    if user_data:
        text = """ 
        У вас имеется активная игра, завершите игру чтобы выбрать новую категорию.
        """
        ...
        tg_id = user_id
        guess_film = GuessedFilmManager().get_guessed_films_ids(tg_id)
        film = FilmManager().get_random_film(film_ids=guess_film, category_id=choice)
